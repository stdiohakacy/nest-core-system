...
export const conversationsSaga = function*() {
    ...
    yield axios.get("http://localhost:3000/api/v1/auth/chat/conversations?perPage=20&page=1&orderBy=createdAt&orderDirection=asc", { headers }).then(response => conversations = response.data.data)
    // yield put(messagesLoaded(conversations[0].id, conversations[0].messages, false, null));
    yield put({
        type: 'CONVERSATIONS_LOADED',
        payload: {
            conversations,
            selectedConversation: conversations[0].conversationId
        }
    });
}
